...
onMounted(() => {
  ...
  if (wrap) {
    const onScroll = () => {
      showBackTop.value = wrap.scrollTop > 200;
    };
...
.back-top-custom {
  ...
  cursor: pointer;
  background-color: var(--el-backtop-bg-color, #fff);
  border-radius: 50%;
  box-shadow: 0 0 6px rgb(0 0 0 / 12%);
  transition: var(--el-transition-duration-fast);
}

.back-top-custom:hover {
  color: #fff;
  background-color: var(--el-backtop-hover-bg-color, var(--el-color-primary));
}
</style>
