...
- [x] 5.6 菜单管理递归删除与权限清理
  - 已处理：删除菜单时递归删除子孙菜单，并同步清理所有相关角色菜单权限。

- [x] 5.7 菜单管理防止错误父级
  - 已处理：新增/修改菜单时校验父级存在，禁止选择自身、下级菜单或按钮作为上级菜单。

- [x] 5.8 动态菜单路由支持多级结构
  - 已处理：后端动态路由改为递归构建；非 admin 用户会自动补齐已授权菜单的父级链路。

- [x] 5.9 菜单管理搜索与状态切换细节修复
  - 已处理：菜单搜索命中子节点时保留父级链路；状态切换 loading 改为按菜单 id 记录；按钮菜单也可在编辑弹窗里修改状态。
...
- [x] 7.3 `pnpm-lock.yaml` 曾被依赖安装整理出较大差异
  - 已处理：删除 AI Chat 后已重新生成 lockfile，移除了 `deep-chat` 相关锁定项。

- [x] 7.4 本地工具目录忽略规则
  - 已处理：`.claude` 和 `.codex` 均已加入 `.gitignore`，避免本地 agent/worktree 配置误提交。
